...
Coord get_nlibs_of_group(const Board *b, group_t id, int k, Coord *libs) {
  if (k > 1 && libs == NULL) 
	  {//error("To get >= 2 libs, libs must not be NULL!");
  }
  ...
  if (lib_count < k) 
	  {
		  //error("The liberty count is %d and cannot get %d liberty points!\n", lib_count, k);
  }
  TRAVERSE(b, id, c) {
    FOR4(c, _, cc) {
...
Coord board_group_other_lib(const Board *b, group_t id, Coord to) {
  ...
  int lib_count = b->_groups[id].liberties;
  if (lib_count < 2) 
	  {//error("The liberty count is %d and cannot get the other liberty point!\n", lib_count);
  }
  TRAVERSE(b, id, c) {
    FOR4(c, _, cc) {
